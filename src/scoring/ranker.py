...
def compute_score(
    ...
    2. Granger sig (25%):     normalize(-log10(granger_pvalue), 0, 10)
    3. OOS robustness (35%):  normalize(oos_r2, 0, 0.3)
    4. Effect size (15%):     normalize(-log10(granger_pvalue) * oos_r2, 0, 3)
    """
    # Component 1: MI strength
    mi_norm = _clip_normalize(pair.mi, 0.0, max_mi)

    # Component 2: Granger significance (-log10 transform)
    granger_log = -np.log10(max(pair.granger_pvalue, 1e-15))
    granger_norm = _clip_normalize(granger_log, 0.0, 10.0)

    # Component 3: OOS robustness
    oos_norm = _clip_normalize(pair.oos_r2, 0.0, 0.3)

    # Component 4: Effect size (combined significance × predictive power)
    effect = granger_log * max(pair.oos_r2, 0.0)
    effect_norm = _clip_normalize(effect, 0.0, 3.0)

    # Weighted sum → scale to 0-10
    raw_score = (
        SCORE_WEIGHT_MI * mi_norm
        + SCORE_WEIGHT_GRANGER * granger_norm
    ...
    return ScoredPair(
        ...
        granger_component=round(granger_norm * SCORE_WEIGHT_GRANGER * 10, 2),
